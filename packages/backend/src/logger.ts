...
const getLoggerLevel =
  (label: string, level: LogLevel) =>
  (message: string, ...moreObjects: any[]) => {
    const extractData: ExtraData = { label, moreObjects };
    log.log(level, message, { label, [EXTRA_DATA]: extractData });
  };

export const getLogger = (label: string) =>
  Object.freeze(
    Object.fromEntries(logLevels.map((lvl) => [lvl, getLoggerLevel(label, lvl)])) as {
      [K in LogLevel]: ReturnType<typeof getLoggerLevel>;
    },
  );
